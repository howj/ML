...
# Weights init from the XOR slides
...
def SGDTrain(input_size, hiddenNodes, output_size, num_epochs, X, Y, batch_size, Xtest, Ytest, Xdev, Ydev):
    minTrainMSE = 100.0
    minTrainMCE = 100.0
    minTestMSE = 100.0
    minTestMCE = 100.0
    ...
    opt = optim.SGD(my_model.parameters(), lr=learning_rate, momentum=0.9, weight_decay = 0.0001)
...
Xaxis, TrainMSE, TrainMCE, TestMSE, TestMCE, DevMSE, DevMCE = SGDTrain(784, 100, 10, iters, Xtrain, Ytrain2, batch_size,
                                                                       Xtest, Ytest2, Xdev, Ydev2) # Correct input size?
...
fig = plt.figure()
train, = plt.plot(Xaxis[20:], TrainMCE[20:], label="Train error")
test, = plt.plot(Xaxis[20:], TestMCE[20:], label="Test error")
dev, = plt.plot(Xaxis[20:], DevMCE[20:], label="Dev error")
